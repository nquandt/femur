...
public class ExtendedMarkdownParser : MarkdownParser
{
    ...
    public ExtendedMarkdownParser(Stream stream, int bufferSize = 4096, bool leaveOpen = false)
    ...
    /// <returns>An ExtendedMarkdownDocumentNode containing parsed markdown and frontmatter.</returns>
    public static new ExtendedMarkdownDocumentNode Parse(Stream stream, bool leaveOpen = false)
    {
        using var parser = new ExtendedMarkdownParser(stream, leaveOpen: leaveOpen);
        return parser.Parse();
    }
